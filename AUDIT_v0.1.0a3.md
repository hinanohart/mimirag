...
| `models.pool_token_stream` was exported from `encoders.fake.__all__` but derived its output dimension from `tokens.max()`, so two calls with different max-IDs would yield non-comparable vectors that silently mis-fit a FAISS index built with the first dim. | MEDIUM | `encoders/fake.py`, `models.py` | `v0.1.0a3` — removed both from `encoders/fake.py`'s `__all__` AND from the re-export chain (the `from mimirag.models import ... pool_token_stream` line was deleted, so `from mimirag.encoders.fake import pool_token_stream` now raises `ImportError`). The helper itself lives on at `mimirag.models.pool_token_stream` (callable but no longer publicly re-exported); the dim-drift trap is suppressed by visibility, not yet fixed at the implementation level. A fixed-vocab `pool_token_stream_fixed(n_codebooks, vocab_size)` helper is queued in `TODO_v0.2.md` for `v0.1.1` as the actual code fix. |
